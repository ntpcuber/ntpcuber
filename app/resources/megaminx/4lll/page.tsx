...
function AlgCard({ img, alg, alt, wide }: MegaCase) {
  return (
    <div className="group bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden hover:border-indigo-500 transition flex flex-col h-full">
      <div className={`bg-neutral-100 dark:bg-neutral-950 ${wide ? 'aspect-video' : 'aspect-square'} flex items-center justify-center shrink-0`}>
...
export default function FourLLLPage() {
  ...
  return (
    <div className="bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 font-sans" style={{ scrollPaddingTop: '120px' }}>
      ...
      {/* ── Sticky section nav ── */}
      <nav className="sticky top-[60px] z-40 bg-white/90 dark:bg-neutral-950/90 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-800 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-3 flex gap-4 md:gap-8 overflow-x-auto items-center"
          ...
          {NAV_LINKS.map(link => (
            <a
              key={link.href}
              href={link.href}
              className="text-neutral-500 dark:text-neutral-400 hover:text-indigo-400 transition whitespace-nowrap text-xs md:text-sm font-semibold uppercase tracking-wider py-1"
            >
